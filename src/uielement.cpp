...
UiElement* UiElement::Factory(uint8_t data[], size_t data_len) {\
  ...
  if (data_len > 0 && UiElement::ui_element_codes[data[0]] == data[0]) {
    ...
    LOG_DEBUG_ARGS("Factory recognised code %X", data[0]);
    ...
    case ColourType::kConstant:
      if (data_len >= 4) {
        ...
        *bytes_used += 4;
        ...
        LOG_DEBUG_ARGS("kConstant Colour provider providing %x", colour);
      ...
      if (data_len >= 10) {
        ...
        *bytes_used += 10;
        ...
        return [=](ProcessData* data){
          ...
          double multiple = std::min(std::max(data->getDoubleData(type), static_cast<double>(min_val)), static_cast<double>(max_val)) / (max_val - min_val);
...
DataType UiElement::getDataType(uint8_t data[], size_t data_len, size_t* bytes_used) {
  ...
  LOG_DEBUG("Data too small to supply DataType");
